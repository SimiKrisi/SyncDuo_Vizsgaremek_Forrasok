...
namespace ApiForGame
{
    internal class GameApi
    {
        ...
        private static readonly Regex[] ForbiddenPatterns =
        {
            ...
            new Regex("k[\\W_]*[u���][\\W_]*r[\\W_]*v[\\W_]*a", RegexOptions.IgnoreCase),
        ...
        private async Task<string> SendRequestAsync(HttpMethod method, string endpoint, object data = null)
        {
            if (IsOffline())
            {
                ...
                UnityEngine.Debug.LogWarning($"[API] Offline �llapot. K�r�s kihagyva: {endpoint}");
                return string.Empty;
            }

            if (IsApiTemporarilyBlocked())
            {
                UnityEngine.Debug.LogWarning($"[API] �tmeneti tilt�s akt�v. K�r�s kihagyva: {endpoint}");
                return string.Empty;
            }

            string url = $"{server}/{endpoint}";

            // --- GET / DELETE eset�n query string gener�l�s ---
            ...
            using (UnityEngine.Networking.UnityWebRequest request = new UnityEngine.Networking.UnityWebRequest(url, method.Method))
            {
                ...
                // K�r�s ind�t�sa �s v�rakoz�s aszinkron m�don
                ...
                else
                {
                    ...
                    // Ha a telefonon hiba van, a Logcatben most m�r T�K�LETESEN fogjuk l�tni!
                    UnityEngine.Debug.LogError($"[API HIBA] V�gpont: {endpoint} | Ok: {request.error} | V�lasz: {request.downloadHandler.text}");
                    return "";
                }
            }
        ...
        public async Task<string> UpdateUser(string userId, object fields)
        {
            ...
            // A fields objektum tulajdons�gait �tm�soljuk a sz�t�rba
            foreach (var prop in fields.GetType().GetProperties())
            {
                dict[prop.Name] = prop.GetValue(fields);
            }
            ...
            // Manu�lisan hozz�adjuk a hi�nyz� ID-t
            dict["user_id"] = userId;
        ...
        /// <summary>
        /// Lek�ri a felhaszn�l�t a Google azonos�t�ja alapj�n (Fi�k rekonstrukci�hoz)
        /// </summary>
        public async Task<string> SelectUserByGoogleId(string googleId)
        {
            // Fontos: A PHP fejleszt�nek biztos�tania kell, hogy a select.php (vagy egy �j v�gpont)
            // k�pes legyen visszaadni a 'user_id'-t, ha csak egy 'google_id'-t kap bemenetk�nt!
